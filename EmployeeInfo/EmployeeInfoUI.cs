...
namespace EmployeeInfo
{
    public partial class EmployeeInfoUI : Form
    {
        ...
        private List<Employee> GetAllEmployeeDetails()
        {
            ...
            aConnection.Open();
            string query = string.Format("SELECT * FROM t_employee");
        ...
        private string UpdateEmployee(Employee employee)
        {
                ...
                aConnection.ConnectionString = connectionString;
                aConnection.Open();
                string query = string.Format("UPDATE t_employee SET name='{0}',address='{1}',salary={2} WHERE id={3}", employee.Name, employee.Address, employee.Salary, employeeId);
        ...
        private string SaveEmployee(Employee employee)
        {
            ...
            else
            {
                aConnection.ConnectionString = connectionString;
                aConnection.Open();
                string query = string.Format("INSERT INTO t_employee VALUES('{0}','{1}','{2}',{3})", employee.Name, employee.Address, employee.Email, employee.Salary);
        ...
        private bool IsThisEmailAlreadyused(string email)
        {
            aConnection.ConnectionString = connectionString;
            aConnection.Open();
            string query = string.Format("SELECT * FROM t_employee WHERE email='{0}'", email);
        ...
        private Employee GetEmployeeById(int id)
        {
            aConnection.ConnectionString = connectionString;
            aConnection.Open();
            string query = string.Format("SELECT * FROM t_employee WHERE id='{0}'", id);
